...
public class LittleDataStoreImpl implements LittleDataStore {
	...
	private TransactionImpl checkTransaction() {
		TransactionImpl t = getTransaction();
		if (t == null) {
	...
	public TransactionImpl getOrOpenTransaction() { 
		TransactionImpl currentTransaction = getTransaction();
		if (currentTransaction == null) {
			currentTransaction = (TransactionImpl) transactionService.openTransaction();
		}
		return currentTransaction;
	}

	private TransactionImpl getTransaction() {
		return (TransactionImpl) transactionService.getTransaction();
	}
